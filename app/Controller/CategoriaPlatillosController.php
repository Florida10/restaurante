	...
	public function add() {
		if ($this->request->is('post')) {
			...
			if ($this->CategoriaPlatillo->save($this->request->data)) {
				$this->Flash->success('La categoria platillo a sido creado.');
 ...
 */
	public function edit($id = null) {
		if (!$this->CategoriaPlatillo->exists($id)) {
			throw new NotFoundException('Invalid categoria platillo');
		}
		if ($this->request->is(array('post', 'put'))) {
			if ($this->CategoriaPlatillo->save($this->request->data)) {
				$this->Flash->success('The categoria platillo has been saved.');
				return $this->redirect(array('action' => 'index'));
			} else {
				$this->Flash->error('The categoria platillo could not be saved. Please, try again.');
			}
		} else {
			$options = array('conditions' => array('CategoriaPlatillo.' . $this->CategoriaPlatillo->primaryKey => $id));
 ...
 */
	public function delete($id = null) {
		...
		if ($this->CategoriaPlatillo->delete()) {
			$this->Flash->success('The categoria platillo has been deleted.');
		} else {
			$this->Flash->error('The categoria platillo could not be deleted. Please, try again.');
		}
		return $this->redirect(array('action' => 'index'));
	}
}
